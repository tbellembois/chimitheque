...
@auth.requires_login()
def create():
    db.physical_state.label.default = ''
    form = crud.create(db.physical_state, next=URL(request.application, 'physical_state', 'list'))
...
@auth.requires_login()
def update():
    form = crud.update(db.physical_state,
                       request.args(0),
...
@auth.requires_login()
def read():
    form = crud.read(db.physical_state, request.args(0))
    return dict(form=form)
...
@auth.requires_login()
def delete():
    form = crud.delete(db.physical_state,
                       request.args[0],
                        next=URL(request.application,
                                 request.controller,
                                 'list',
